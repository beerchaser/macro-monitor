...
#   - [재보정] Repo 판단 임계를 GCF 기준(+20/+30bp)에서 SOFR-IORB 기준으로 교체.#!/usr/bin/env python3
# ========================================================================
# macro-monitor 자동 업데이트 스크립트 v17.5
# v17.5 (8/16):
#   - [구조수정] patch_cpi를 문장-재읽기 방식에서 row-id 범위 교체 방식으로 전환.
#     이전 방식은 note가 스크립트가 쓴 정형 문장과 정확히 같아야만 재매칭됐다.
#     실제로 note를 수동 정오표 문구로 바꾸자(8/15) note 패치만 조용히 실패하고
#     val만 새 값으로 덮어써져 val≠note≠threshold인 사고가 재발했다(8/16 확인).
#     이제 <tr id=row-cpi>...</tr> 범위를 먼저 자르고 그 안에서 class로만
#     찾으므로 이전 문구가 무엇이었든 무관하게 동작한다.
#   - [수정] note의 시리즈 라벨이 v17.4 이후에도 SA명(CPIAUCSL/CPILFESL)으로
#     남아 있던 것을 실제 사용 중인 NSA명(CPIAUCNS/CPILFENS)으로 정정.
# v17.4 (8/15):
#   - [긴급수정] fetch_cpi가 SA 시리즈(CPIAUCSL/CPILFESL)로 YoY를 계산해 실제 BLS
#     발표치와 어긋남(헤드라인 -10bp, 코어 -30bp). BLS 보도자료 관행인 NSA
#     시리즈(CPIAUCNS/CPILFENS)로 교체. 7월 실측 대조: 헤드라인 3.4%·코어 2.5%.
...
# v17.5: 문장 전체를 다시 읽는 방식(v17.1~17.4)은 "이전 note가 정확히
# 스크립트가 쓴 그 문장일 것"을 전제한다. 수동으로 다른 문구를 넣는 순간
# (실제 8/15~16 발생: 정오표 설명문을 손으로 적었더니 note 패치만 실패하고
# val만 새 값으로 덮어써져, val과 note/threshold가 서로 다른 값을 말하는
# 사고가 재현됨) 깨진다. row-id로 범위를 먼저 자르고 그 안에서 val/note
# 클래스만으로 찾아 교체 — 이전 문구가 무엇이었든 무관하게 동작한다.
CPI_ROW_RX = r'(<tr[^>]*id="row-cpi"[^>]*>)(.*?)(</tr>)'


def patch_cpi(html, cpi):
    if not cpi:
        return html
    m = re.search(CPI_ROW_RX, html, re.S)
    if not m:
        print("    ⚠️  미매칭: CPI row 자체를 못 찾음")
        return html
    body = m.group(2)
    ...
    note_text = (f'{cpi["date"]} BLS {cpi["month"]} 헤드라인 {cpi["headline"]:.1f}%·'
                 f'Core {cpi["core"]:.1f}% · FRED CPIAUCNS/CPILFENS(NSA) 산출')
    new_body, n2 = re.subn(
        r'(<span class="verify-note">).*?(</span>)',
        lambda mm: mm.group(1) + note_text + mm.group(2),
        new_body, count=1, flags=re.S)

    if n1 == 0:
        print("    ⚠️  val 미매칭: CPI")
    if n2 == 0:
        print("    ⚠️  note 미매칭: CPI")
    if n1 and n2:
        html = html[:m.start()] + m.group(1) + new_body + m.group(3) + html[m.end():]
        print(f'    ✅ CPI val+note (row-bound)')

    register_anchor("CPI row", CPI_ROW_RX)
    return html
